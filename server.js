    ...
    Object.keys(wingoStates).forEach(async (mode) => {
        ...
        if (wingoStates[mode].timeLeft <= 0) {
            ...
            roundBets.forEach(async (bet) => {
                let isWin = false;
                let profitMultiplier = 2; // ডিফল্ট ২ গুণ প্রফিট (কালার এবং বিগ-স্মলের জন্য)
...
io.on('connection', (socket) => {
    ...
    console.log("Player socket pipe attached to VIP WinGo Casino Engine!");
});
...
app.post('/api/wingo-bet', async (req, res) => {
    ...
    // ওস্তাদ! ফ্রন্টএন্ড থেকে আসা 'WinGo 30s' বা '1m' টেক্সটকে ব্যাকএন্ডের সেকেন্ড কি-তে কনভার্ট করার কিংস বর্ম
    let numericMode = 30;
    if (String(mode).includes("60") || String(mode).includes("1")) numericMode = 60;
    if (String(mode).includes("180") || String(mode).includes("3")) numericMode = 180;
    if (String(mode).includes("300") || String(mode).includes("5")) numericMode = 300;

    // শেষ ৫ সেকেন্ডে বাজি ধরা কড়া নিয়মে লকড (লেট-বেট প্রোটেকশন বর্ম সচল লক ভাই!)
    if (wingoStates[numericMode] && wingoStates[numericMode].timeLeft <= 5) {
